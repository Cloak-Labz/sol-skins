...
async function createSkinTemplates(): Promise<SkinTemplate[]> {
  const skins = [
    // AK-47 Skins
    { weapon: 'AK-47', skinName: 'Redline', rarity: 'Rare', condition: 'Field-Tested', basePriceUsd: 45.20, collection: 'The Phoenix Collection' },
    { weapon: 'AK-47', skinName: 'Vulcan', rarity: 'Epic', condition: 'Factory New', basePriceUsd: 125.50, collection: 'The Phoenix Collection' },
    { weapon: 'AK-47', skinName: 'Fire Serpent', rarity: 'Legendary', condition: 'Field-Tested', basePriceUsd: 450.00, collection: 'The Cobblestone Collection' },
    { weapon: 'AK-47', skinName: 'Blue Laminate', rarity: 'Common', condition: 'Minimal Wear', basePriceUsd: 2.50, collection: 'The Dust Collection' },
    { weapon: 'AK-47', skinName: 'Jaguar', rarity: 'Uncommon', condition: 'Well-Worn', basePriceUsd: 8.75, collection: 'The Dust Collection' },
    
    // AWP Skins
    { weapon: 'AWP', skinName: 'Dragon Lore', rarity: 'Legendary', condition: 'Factory New', basePriceUsd: 2500.00, collection: 'The Cobblestone Collection' },
    { weapon: 'AWP', skinName: 'Asiimov', rarity: 'Epic', condition: 'Field-Tested', basePriceUsd: 85.30, collection: 'The Phoenix Collection' },
    { weapon: 'AWP', skinName: 'Redline', rarity: 'Rare', condition: 'Minimal Wear', basePriceUsd: 25.40, collection: 'The Phoenix Collection' },
    { weapon: 'AWP', skinName: 'Corticera', rarity: 'Uncommon', condition: 'Field-Tested', basePriceUsd: 12.80, collection: 'The Dust Collection' },
    { weapon: 'AWP', skinName: 'Worm God', rarity: 'Common', condition: 'Battle-Scarred', basePriceUsd: 1.20, collection: 'The Dust Collection' },
    
    // M4A4 Skins
    { weapon: 'M4A4', skinName: 'Howl', rarity: 'Legendary', condition: 'Factory New', basePriceUsd: 1200.00, collection: 'The Phoenix Collection' },
    { weapon: 'M4A4', skinName: 'Asiimov', rarity: 'Epic', condition: 'Field-Tested', basePriceUsd: 65.20, collection: 'The Phoenix Collection' },
    { weapon: 'M4A4', skinName: 'X-Ray', rarity: 'Rare', condition: 'Minimal Wear', basePriceUsd: 18.90, collection: 'The Phoenix Collection' },
    { weapon: 'M4A4', skinName: 'Desert-Strike', rarity: 'Uncommon', condition: 'Field-Tested', basePriceUsd: 5.50, collection: 'The Dust Collection' },
    { weapon: 'M4A4', skinName: 'Tornado', rarity: 'Common', condition: 'Well-Worn', basePriceUsd: 0.85, collection: 'The Dust Collection' },
    
    // Glock Skins
    { weapon: 'Glock-18', skinName: 'Fade', rarity: 'Legendary', condition: 'Factory New', basePriceUsd: 350.00, collection: 'The Phoenix Collection' },
    { weapon: 'Glock-18', skinName: 'Water Elemental', rarity: 'Epic', condition: 'Minimal Wear', basePriceUsd: 45.60, collection: 'The Phoenix Collection' },
    { weapon: 'Glock-18', skinName: 'Steel Disruption', rarity: 'Rare', condition: 'Field-Tested', basePriceUsd: 8.25, collection: 'The Phoenix Collection' },
    { weapon: 'Glock-18', skinName: 'Sand Dune', rarity: 'Common', condition: 'Battle-Scarred', basePriceUsd: 0.15, collection: 'The Dust Collection' },
    
    // USP-S Skins
    { weapon: 'USP-S', skinName: 'Kill Confirmed', rarity: 'Epic', condition: 'Factory New', basePriceUsd: 95.80, collection: 'The Phoenix Collection' },
    { weapon: 'USP-S', skinName: 'Orion', rarity: 'Rare', condition: 'Minimal Wear', basePriceUsd: 15.40, collection: 'The Phoenix Collection' },
    { weapon: 'USP-S', skinName: 'Forest Leaves', rarity: 'Uncommon', condition: 'Field-Tested', basePriceUsd: 2.10, collection: 'The Dust Collection' },
  ];
  ...
  for (const skin of skins) {
    ...
    template.imageUrl = `https://steamcommunity-a.akamaihd.net/economy/image/class/730/${Math.random().toString(36).substr(2, 9)}`;
...
async function createLootBoxSkinPools(lootBoxTypes: LootBoxType[], skinTemplates: SkinTemplate[]) {
  const pools = [];
...
async function createUserSkins(users: User[], skinTemplates: SkinTemplate[], lootBoxTypes: LootBoxType[]) {
  const userSkins = [];
  ...
  for (const user of users) {
    ...
    for (let i = 0; i < numSkins; i++) {
      const randomSkin = skinTemplates[Math.floor(Math.random() * skinTemplates.length)];
      const randomLootBox = lootBoxTypes[Math.floor(Math.random() * lootBoxTypes.length)];
      
      const userSkin = new UserSkin();
      userSkin.userId = user.id;
      userSkin.skinTemplateId = randomSkin.id;
      userSkin.nftMintAddress = `NFT${Math.random().toString(36).substr(2, 9)}${Math.random().toString(36).substr(2, 9)}`;
      userSkin.lootBoxTypeId = randomLootBox.id;
      userSkin.openedAt = new Date(Date.now() - Math.random() * 30 * 24 * 60 * 60 * 1000); // Random date in last 30 days
...
async function createTransactions(users: User[], lootBoxTypes: LootBoxType[]) {
  ...
  for (const user of users) {
    ...
    for (let i = 0; i < numTransactions; i++) {
      ...
      transaction.transactionType = Math.random() > 0.7 ? 'buyback' : 'open_case';
      ...
      transaction.lootBoxTypeId = lootBoxTypes[Math.floor(Math.random() * lootBoxTypes.length)].id;
      transaction.txHash = `tx${Math.random().toString(36).substr(2, 9)}${Math.random().toString(36).substr(2, 9)}`;
      transaction.blockSlot = Math.floor(Math.random() * 1000000) + 1000000;
      transaction.confirmedAt = new Date(Date.now() - Math.random() * 30 * 24 * 60 * 60 * 1000);
      transaction.status = Math.random() > 0.1 ? 'confirmed' : 'pending'; // 90% confirmed
