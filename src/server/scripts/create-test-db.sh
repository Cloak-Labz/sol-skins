...
if ! docker ps | grep -q "d3-postgres"; then
    echo "Error: PostgreSQL container 'd3-postgres' is not running"
...
docker exec -i d3-postgres psql -U postgres -c "CREATE DATABASE sol_skins_test;" 2>/dev/null

if [ $? -eq 0 ]; then
    echo "✅ Test database 'sol_skins_test' created successfully!"
else
    # Check if database already exists
    if docker exec -i d3-postgres psql -U postgres -lqt | cut -d \| -f 1 | grep -qw "sol_skins_test"; then
