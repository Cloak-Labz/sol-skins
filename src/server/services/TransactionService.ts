...
export class TransactionService {
  ...
  async getUserTransactions(userId: string, options: {
    ...
    const [transactions, total] = await this.transactionRepository.findByUser(userId, {
      skip,
      take: limit,
    ...
    const summary = await this.transactionRepository.getUserTransactionSummary(userId);
  ...
  async getTransactionById(userId: string, transactionId: string) {
    ...
    if (transaction.userId !== userId) {
      throw new AppError('Unauthorized to view this transaction', 403, 'UNAUTHORIZED');
    }
