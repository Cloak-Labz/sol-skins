...
export class InventoryService {
  ...
  async getUserInventory(userId: string, options: {
    ...
    const [userSkins, total] = await this.userSkinRepository.findByUser(userId, {
      skip,
      take: limit,
  ...
  async sellSkinViaBuyback(userId: string, skinId: string, minAcceptablePrice?: number) {
    ...
    if (!userSkin.isInInventory || userSkin.soldViaBuyback) {
      throw new AppError('Skin is not available for sale', 400, 'SKIN_NOT_AVAILABLE');
    }

    const currentPrice = userSkin.currentPriceUsd || userSkin.skinTemplate?.basePriceUsd || 0;
    const buybackPrice = currentPrice * 0.85; // 85% of current market value

    if (minAcceptablePrice && buybackPrice < minAcceptablePrice) {
      throw new AppError(
        `Current buyback price (${buybackPrice.toFixed(2)}) is below minimum acceptable price`, 
        400, 
        'PRICE_TOO_LOW'
      );
    }
