...
import { ArrowLeft, Loader2, User, Mail, Wallet, Calendar, TrendingUp, Package } from "lucide-react"
...
export default function ProfilePage() {
  ...
  const { user, isConnected, refreshUser } = useUser()
              ...
              <div className="mt-6 p-4 bg-muted/50 rounded-lg">
                <div className="flex items-center justify-between">
                  ...
                  <span className={`text-xl font-bold ${
                    (user.totalEarned || 0) - (user.totalSpent || 0) >= 0 
                      ? "text-green-400" 
                      : "text-red-400"
                  }`}>
                    {((user.totalEarned || 0) - (user.totalSpent || 0)) >= 0 ? "+" : ""}
                    {formatCurrency((user.totalEarned || 0) - (user.totalSpent || 0))}
                  </span>
                </div>
              </div>
            </CardContent>
