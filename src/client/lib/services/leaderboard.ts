...
class LeaderboardService {
  ...
  async getLeaderboard(filters?: LeaderboardFilters): Promise<{
    ...
    console.log('LeaderboardService: Response success:', response?.data?.success);
    console.log('LeaderboardService: Response data:', response?.data?.data);
    console.log('LeaderboardService: Full response structure:', JSON.stringify(response, null, 2));
    
    // Check if response is already the data array (from interceptor) or if it's the full response
    if (Array.isArray(response)) {
      console.log('LeaderboardService: Response is already an array, returning directly');
